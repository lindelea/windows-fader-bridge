...
enum { Nav = 500, Tracks = 600, Prev, Next, Hide, Menu, TargetList = 620, Search, Categories,
    ...
    Sensitivity, Seconds, Layer, Language = 650, TrayClose, Startup, InPort, OutPort, Profile,
    Touch, Lcd, Meters, Refresh, Connect, Preset, Debug, Log, Trace, SettingsFolder, AboutLink,
...
{ Close(); for(auto f:fonts_)if(f)DeleteObject(f);if(iconFont_)DeleteObject(iconFont_); if(background_)DeleteObject(background_); if(field_)DeleteObject(field_); }
...
void MackieDesktop::Build()
{
    ...
    if(page_==1){Add(WC_COMBOBOXW,L"",Combo,Language);Fill(window_,Language,{L"简体中文",L"English"},zh_?0:1);Toggle(TrayClose,T(L"关闭窗口后后台运行",L"Run in background when closed"));Toggle(Startup,T(L"随 Windows 启动",L"Start with Windows"));}
    if(page_==2)
    {
        ...
        Button(Refresh,T(L"刷新",L"Refresh"));Button(Connect,L"");Button(SaveDeviceButton,T(L"保存设备",L"Save device"));DeviceChoices();Ports();
    }
    if(page_>=3&&page_<=5)
    {
    ...
    if(page_==6){Button(Log,T(L"打开日志目录",L"Open log folder"));Button(SettingsFolder,T(L"打开配置目录",L"Open settings folder"));Toggle(Trace,T(L"详细 MIDI 日志",L"Detailed MIDI logging"));Button(Preset,T(L"应用 Windows 80 键预设",L"Apply Windows 80-key preset"));}
    if(page_==7)Button(AboutLink,T(L"打开项目主页",L"Project website"));
...
void MackieDesktop::Layout()
{
    ...
    if(page_==0){Place(Connect,width_-204,48,172,36);Place(Tracks,x,242,w,std::max(110,bottom-390));Place(Prev,x,bottom-138,122,32);Place(Next,x+132,bottom-138,122,32);}
    if(page_==1){Place(Language,width_-280,164,224,200);Place(TrayClose,width_-112,282,56,30);Place(Startup,width_-112,398,56,30);}
    if(page_==2)
    {
        ...
        int col=(right-48)/3;Place(Touch,split+24,487,52,28);Place(Lcd,split+24+col,487,52,28);Place(Meters,split+24+col*2,487,52,28);
    ...
    if(page_==6){Place(Log,x+24,184,216,36);Place(Trace,width_-112,290,56,30);Place(SettingsFolder,x+24,410,220,36);Place(Preset,x+260,410,310,36);}
    if(page_==7)Place(AboutLink,x+24,420,210,38);
}
...
void MackieDesktop::Paint(HDC dc)
{
    ...
    if(page_==0)
    {
        ...
        Text(dc,L"CH "+std::to_wstring(app_.Surface().BankStart()+1)+L"–"+std::to_wstring(app_.Surface().BankStart()+8),x+272,bottom-136,w-272,28,Muted,1,DT_RIGHT|DT_SINGLELINE|DT_VCENTER);
        Panel(dc,x,bottom-90,w,86,Card,Edge);
        ...
        Text(dc,media.available?(media.playing?T(L"正在播放",L"NOW PLAYING"):T(L"已暂停",L"PAUSED")):T(L"媒体待机",L"MEDIA IDLE"),x+20,bottom-80,w-40,18,Amber,4);
        Text(dc,media.available&&!media.title.empty()?media.title:T(L"等待播放器",L"Waiting for a player"),x+20,bottom-60,w-224,25,Ink,0);
        Text(dc,media.available?media.artist:L"",x+20,bottom-32,w-224,20,Muted,1);
        Text(dc,time.available?mackie::TimeText(time.elapsedSeconds):mackie::TimeText(app_.Surface().TimeDisplaySeconds),width_-214,bottom-63,158,27,Ink,3,DT_RIGHT|DT_SINGLELINE|DT_VCENTER);
        Text(dc,time.available?(L"/ "+mackie::TimeText(time.durationSeconds)):T(L"系统时间",L"SYSTEM CLOCK"),width_-214,bottom-33,158,20,Muted,1,DT_RIGHT|DT_SINGLELINE|DT_VCENTER);
    }
    if(page_==1)
    {
        const wchar_t* aZh[]={L"界面语言",L"关闭窗口时保留后台运行",L"随 Windows 启动"};
        const wchar_t* aEn[]={L"Interface language",L"Keep running when the window closes",L"Start with Windows"};
        for(int i=0;i<3;++i){Panel(dc,x,132+i*116,w,100,Card,Edge);Text(dc,T(aZh[i],aEn[i]),x+24,132+i*116,w-(i==0?300:140),100,Ink,3);}
    }
    if(page_==2)
    {
        int split=x+264,right=width_-32-split;
        ...
        int col=(right-48)/3;Text(dc,T(L"触摸保护",L"Touch guard"),split+24,459,col,22,Muted,1);Text(dc,L"LCD",split+24+col,459,col,22,Muted,1);Text(dc,T(L"峰值表",L"Meters"),split+24+col*2,459,col,22,Muted,1);
    ...
    if(page_==6)
    {
        ...
        Panel(dc,x,366,w,104,Card,Edge);Text(dc,T(L"配置与预设",L"Configuration & presets"),x+24,375,w-48,28,Ink,3);
    }
    if(page_==7)
    {
...
void MackieDesktop::DrawItem(DRAWITEMSTRUCT* d)
{
    ...
    if(d->CtlType==ODT_BUTTON)
    {
        ...
        auto background=CreateSolidBrush(nav?Side:Bg);RECT whole{0,0,S(w),S(h)};FillRect(d->hDC,&whole,background);DeleteObject(background);
        if(d->itemState&ODS_SELECTED)fill=primary?RGB(214,158,65):Edge;
        Panel(d->hDC,0,0,w,h,fill,nav?fill:Edge);
        Text(d->hDC,Read(d->hwndItem),nav?16:10,0,w-(nav?28:20),h,disabled?Muted:active?Amber:primary?Bg:Ink,0,DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS|(nav?DT_LEFT:DT_CENTER));
    ...
    else if(d->CtlType==ODT_LISTVIEW&&d->itemID!=static_cast<UINT>(-1))
    {
        ...
        else if(d->CtlID==Tracks&&d->itemID<trackKeys_.size())
        {
            ...
            Text(d->hDC,s?s->name:T(L"等待通道更新",L"Waiting for channel"),60,9,w-370,26,Ink);
            if(s)
            {
                ...
                Text(d->hDC,role,60,35,w-370,21,Muted,1);
...
void MackieDesktop::Act(int id,int code)
{
    ...
    case SettingsFolder:OpenFolder(app_.workspace_.root);break;
    case Trace:{bool on=std::any_of(app_.devices_.begin(),app_.devices_.end(),[](const auto& d){return d->midi.Trace;});for(auto& d:app_.devices_)d->midi.Trace=!on;Tick();}break;
    case Preset:
    ...
    case AboutLink:ShellExecuteW(window_,L"open",L"https://github.com/Lindelea/Windows-Fader-Bridge",nullptr,nullptr,SW_SHOWNORMAL);break;
    }
}
void MackieDesktop::Tick()
{
    ...
    if(page_==6)check(Trace,std::any_of(app_.devices_.begin(),app_.devices_.end(),[](const auto& d){return d->midi.Trace;}));
...
LRESULT MackieDesktop::Message(UINT m,WPARAM a,LPARAM b)
{
    ...
    case WM_MEASUREITEM:{auto d=reinterpret_cast<MEASUREITEMSTRUCT*>(b);d->itemHeight=S(d->CtlType==ODT_COMBOBOX?30:d->CtlID==Tracks?64:56);return TRUE;}
