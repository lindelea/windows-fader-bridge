...
constexpr wchar_t Title[] = L"Apollo Bridge for EUCON";
constexpr int ConnectButton = 101, LogsButton = 102, LanguageButton = 103, ArmButton = 104,
              MonitorButton = 105, AllChannelsButton = 106, SafetyButton = 107;
...
bool OtherTestAdapterRunning()
{
    ...
    do
    {
        if (_wcsicmp(entry.szExeFile, L"WindowsFaderBridge.exe") == 0 ||
            _wcsicmp(entry.szExeFile, L"EuConIO.exe") == 0 || _wcsicmp(entry.szExeFile, L"EuConApp.exe") == 0)
        {
            found = true;
            break;
        }
...
struct App
{
    HWND window = nullptr, connect = nullptr, logs = nullptr, language = nullptr, arm = nullptr,
         monitorArm = nullptr, allChannels = nullptr, safety = nullptr;
    apollo::Observer observer;
    ...
    void Fonts()
    {
        ...
        buttonFont = CreateFontW(-Px(14), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, 0, 0,
                                 CLEARTYPE_QUALITY, 0, L"Segoe UI");
        for (HWND button : {connect, logs, language, arm, monitorArm, allChannels, safety})
            if (button)
    ...
    void Tick()
    {
            ...
            monitoring.error != lastMonitorError)
        {
            apollo::Log("monitor-control epoch=" + std::to_string(monitoring.epoch) +
                        ...
                        " confirmed=" + std::to_string(monitoring.confirmed) + " " +
        ...
        {
            apollo::Log("control epoch=" + std::to_string(control.epoch) + " confirmed=" +
        ...
        if (++tick % 30 == 0)
            adapterConflict = OtherTestAdapterRunning();
    ...
    void Command(int id)
    {
        ...
        else if (id == ConnectButton && !eucon)
        {
            ...
            try
            {
                eucon = std::make_unique<apollo::ApolloEucon>(snapshot, controller, monitorController);
                error.clear();
            }
            catch (const std::exception &e)
            {
            ...
            else if (eucon && (all || !selectedKey.empty()))
            {
                ...
                const auto message =
                    ...
                    T(L"\n\n允许 EUCON 控制通道的推子、Mute、Solo、PAN、AUX/Cue "
                      ...
                      L"自己发起的路由切换确认后继续控制；断线或外部能力变化则锁定。控制室独立解锁。",
                      ...
                      L"Confirmed routing continues automatically. Disconnection or external capability "
                      L"changes lock the affected channel. Control room unlock is separate.");
        ...
        else if (id == MonitorButton)
        {
                     ...
                     apollo::MonitorEligible(snapshot.monitors.front()))
            {
                ...
                const auto message = T(L"允许 EUCON 控制室改变主监听音量、Mute、Dim、Mono、DIM "
                                       ...
                                       L"configuration changes lock control, "
        ...
        else if (id == LogsButton)
        {
            ...
            {
                const auto folder = std::filesystem::path(base) / L"Apollo Bridge" / L"EUCON" / L"logs";
                CoTaskMemFree(base);
                std::error_code ec;
    ...
    void Paint()
    {
        ...
        if (footer.empty() && !control.error.empty())
        {
            if (control.error == "Sensitive control locked; confirm safety for this channel in the app")
                footer = T(L"敏感操作受保护：请先为所选通道确认“敏感操作保护”。",
                     ...
                     "Channel identity, capabilities or connection changed; this channel was locked")
                footer = T(L"通道身份、能力或连接发生变化，已锁定受影响通道。",
                           L"Channel identity, capabilities or connection changed; affected channel locked.");
            else if (control.error == "Control gesture expired before write")
                footer = T(L"操作超时，未发送；该通道已锁定，请重新开放。",
                           L"Operation timed out before send; channel locked. Enable it again to continue.");
            else
        ...
        if (footer.empty() && !monitoring.error.empty())
            footer = monitoring.error == "Control gesture expired before write"
                         ? T(L"操作超时，未发送；控制室已锁定，请重新解锁。",
                             L"Operation timed out before send; control room locked. Unlock to continue.")
                         : apollo::Wide(monitoring.error);
        if (footer.empty() && !snapshot.connected)
            footer = apollo::Wide(snapshot.status);
        if (footer.empty())
            footer = adapterConflict && !eucon ? T(L"测试前请退出 Windows Fader Bridge 的 EUCON 版。",
                                                   L"Exit Windows Fader Bridge for EUCON before the "
                                                   L"surface test.")
...
LRESULT CALLBACK WindowProc(HWND window, UINT message, WPARAM w, LPARAM l)
{
    ...
    case WM_CREATE:
        ...
        app->Fonts();
        app->adapterConflict = OtherTestAdapterRunning();
        app->observer.Start();
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show)
{
    ...
    bool smoke = false, sdkTextTest = false;
    int observe = 0;
    ...
    LocalFree(arguments);
    if (int(smoke) + int(sdkTextTest) + int(observe != 0) > 1)
    {
    ...
    int result = 0;
    {
        ...
        HWND window = CreateWindowExW(
            0, WindowClass, Title, WS_OVERLAPPEDWINDOW | WS_VSCROLL | WS_CLIPCHILDREN, CW_USEDEFAULT,
